...
import ru.naumen.gintonic.project_builder.EgapProjectBuilder;
...
public class GuiceIndex implements Serializable {
            ...
            protected IStatus run(IProgressMonitor progressMonitor) {
                ...
                for (IProject project : projectsWithEgapNature) {
                    try {
                        ...
                        iProject.build(IncrementalProjectBuilder.FULL_BUILD, EgapIDs.BUILDER, null, progressMonitor);
    ...
    public void addGuiceModule(GuiceModule guiceModule, boolean log) {
        ...
        if (log) {
            EgapPlugin.logInfo("Added " + guiceModule.getPrimaryTypeNameFullyQualified() + " to Guice index "
                    + getIndexInfoShort() + ".");
        }
    }
    ...
    public void updateGuiceModule(GuiceModule guiceModule) {
        removeGuiceModule(guiceModule.getPrimaryTypeNameFullyQualified(), false);
        addGuiceModule(guiceModule, false);
        EgapPlugin.logInfo("Updated " + guiceModule.getPrimaryTypeNameFullyQualified() + " from Guice index "
                + getIndexInfoShort() + ".");
    }
